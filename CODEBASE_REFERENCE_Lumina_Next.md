...
- **Bones generated** (8 total, `npx boneyard-js build`): `dashboard.TodaySummaryWidget` (12 bones), `dashboard.GoalsWidget` (2), `dashboard.CoinsWidget` (3), `page.GoalsPage.grid` (3), `page.ShopPage.grid` (128), `tasks.TaskBoard.kanban` (13), `planner.DailyPlanView` (108–118 responsive), `page.PerformancePage` (422). All written to `src/bones/*.bones.json`.
- **Missing**: `tasks.TaskBoard.list` — list view only renders when `viewMode === 'list'` which requires localStorage state the CLI can't set. Falls back to `SkeletonPrimitive` markup (still functional).
