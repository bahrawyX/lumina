...
--> statement-breakpoint
-- ==========================================================================
-- COLUMN-HEAL RUNS HERE, BEFORE ANYTHING REFERENCES A COLUMN
--
-- It used to run ~400 lines below, after the indexes and the 0021-0026 ALTERs.
-- That ordering is what made this script fail on a real database:
--
--     ERROR: column "position" does not exist (SQLSTATE 42703)
--       at CREATE INDEX "tasks_user_status_position_idx"
--
-- ...and, once that was moved below the indexes only, two more of the same
-- class from the foreign keys:
--
--     column "calendar_id" referenced in foreign key constraint does not exist
--     column "goal_id" referenced in foreign key constraint does not exist
--
-- So it belongs before the CONSTRAINTS too, not just the indexes: this is
-- the first point at which any statement references a column by name.
--
-- `CREATE TABLE IF NOT EXISTS "tasks" (... "position" ...)` is a no-op when
-- `tasks` already exists, so on any database that predates 0022 the column was
-- still missing when the index that references it ran. The heal pass adds it —
-- but it ran afterwards, so it never got the chance.
--
-- The PGlite fixtures never caught this because they start either from an
-- EMPTY database (where CREATE TABLE really does create the column) or from a
-- fixture missing only the two columns the outage named. A database carrying
-- 0000-0019 plus `push` drift is neither.
-- ==========================================================================
