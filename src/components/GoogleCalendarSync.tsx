  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-5 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-semibold text-sm text-gray-900 dark:text-gray-100">Google Calendar</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400">Fetch events for the current view</p>
        </div>
      </div>
      ...
      {status === 'success' && (
        <div className="flex items-start gap-2 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 px-4 py-3">
          ...
          <div className="text-xs text-green-700 dark:text-green-300">
            <p className="font-medium">Fetched successfully</p>
            <p>{count} event{count !== 1 ? 's' : ''} loaded into calendar</p>
          </div>
        </div>
      )}
      ...
      >
        {isSyncing ? <><SpinnerIcon /> Fetching…</> : <><RefreshIcon /> {status === 'success' ? 'Refresh Google Calendar' : 'Load Google Calendar'}</>}
      </button>

      <p className="text-[11px] text-gray-400 dark:text-gray-500 text-center">
        Read-only · Cached in browser · Not stored in database
      </p>
    </div>
  );
}
