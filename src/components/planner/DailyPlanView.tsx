...
export const DailyPlanView: React.FC<DailyPlanViewProps> = ({ onToggleInsights, insightsOpen }) => {
  ...
  const commitQuickAdd = useCallback(() => {
    ...
    const difficultyPriority = { easy: 'low', medium: 'medium', hard: 'high' } as const;
    addTask({ title: trimmed, status: 'todo', priority: difficultyPriority[quickAddDifficulty], difficulty: quickAddDifficulty, durationMinutes: quickAddDuration });
    setQuickAddTitle('');
    setQuickAddDifficulty('medium');
    setQuickAddDuration(30);
    setQuickAddEmojiOpen(false);
